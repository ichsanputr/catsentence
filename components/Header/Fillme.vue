<template>
    <DialogLanguange v-model="dialogLanguange" />
    <div class="flex justify-between color-nextu items-center color-nextu">
        <div class="cursor-pointer flex color-second items-center" @click="navigateTo('/fillme')">
            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current" width="28" height="28" viewBox="0 0 24 24">
                <path
fill-rule="evenodd"
            ...
            <div class="ml-2">
                <div class="text-xs -mb-1">FillMe</div>
                <div class="text-xl sm:text-2xl">Catsentence</div>
            </div>
        </div>
        <div class="flex gap-2 sm:gap-4">
            <v-tooltip location="bottom">
                <template #activator="{ props }">
                    <div class="flex items-center">
                        <div v-bind="props" class="cursor-pointer flex rounded-md" @click="navigateTo('/fillme/how-to-play', {external: true})">
                            <Icon :width="$vuetify.display.mobile ? 18 : 22" icon="mingcute:information-fill" />
                        </div>
                    </div>
                </template>
            ...
            <v-tooltip location="bottom">
                <template #activator="{ props }">
                    <div class="flex items-center">
                        <div v-bind="props" class="cursor-pointer flex rounded-md" @click="navigateTo('/fillme/room/my')">
                            <Icon :width="$vuetify.display.mobile ? 16 : 20" icon="fa6-solid:list" />
                            <div class="ml-1 text-sm capitalize">{{ Languange }}</div>
                        </div>
                    </div>
                </template>
                <span>Room</span>
            ...
            <v-tooltip location="bottom">
                <template #activator="{ props }">
                    <div class="flex items-center">
                        <div v-bind="props" class="cursor-pointer flex rounded-md" @click="dialogLanguange = true">
                            <Icon :width="$vuetify.display.mobile ? 20 : 24" icon="fa-solid:language" />
                            <div class="ml-1 text-sm capitalize">{{ Languange }}</div>
                        </div>
                    </div>
                </template>
            ...
            <v-tooltip location="bottom">
                <template #activator="{ props }">
                    <div class="flex items-center">
                        <div
v-bind="props" class="cursor-pointer flex rounded-md"
                            @click="navigateTo('/fillme/leaderboard')">
                            <Icon :width="$vuetify.display.mobile ? 20 : 24" icon="material-symbols:crown-rounded" />
                            <div class="ml-1 text-sm capitalize">{{ Leaderboard }}</div>
                        </div>
                    </div>
                </template>
            ...
            <div class="flex items-center">
                <div v-bind="prps" class="cursor-pointer flex rounded-md"  @click="navigateTo('/profile', {external: true})">
                    <Icon :width="$vuetify.display.mobile ? 20 : 24" icon="gridicons:user" />
                    <div class="ml-1 text-sm capitalize">{{ username }}</div>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    computed: {
        languange: {
            get() {
                return this.languangeStore.languange;
            },
            set(v) {
                this.languangeStore.languange = v;
            },
        },
        username() {
            return this.typeStore.user?.username
        }
    },
}
</script>